...
#Write header to csv
with open("test.csv", "w", encoding='utf-8', newline='') as file_csv:
...
for data in datas:
	...
	for subject in ["toán", "ngữ văn", "khxh", "khtn", "lịch sử", "địa lí", "gdcd", "sinh học", "vật lí", "hóa học", "tiếng anh"]:
		if subject in score_list:
			subject_name_pos = score_list.index(subject)
			subject_score_pos = subject_name_pos + 1
			subject_score = score_list[subject_score_pos]
